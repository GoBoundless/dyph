...
describe Dyph::Support::Merger do
  let(:merger)         { Dyph::Support::Merger }
  [ lambda { |x| x.to_s } , lambda { |x| x.to_sym }, lambda { |x| Fish.new(x) } ].each do |f|
    describe ".merger" do
      ...
      it "should a non_conflict" do
        result = [Dyph::Outcome::Resolved.new([f.call('a')])]
        expect(merger.merge([f.call('a')],[f.call('a')],[f.call('a')])).to eq result
      end

      it "should a non_conflict" do
        result = [Dyph::Outcome::Resolved.new([f.call('b')])]
        expect(merger.merge([f.call('b')],[f.call('a')],[f.call('a')])).to eq result
      end

      it "should a conflict" do
        result = [Dyph::Outcome::Conflicted.new(left: [f.call("b")], base: [f.call("a")], right: [f.call("c")])]
