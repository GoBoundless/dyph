...
describe Dyph::Differ do
  ...
  let(:resloved)    { Dyph::Outcome::Resolved }
  let(:conflicted)  { Dyph::Outcome::Conflicted }
  # [Dyph::TwoWayDiffers::ResigDiff, Dyph::TwoWayDiffers::HeckelDiff].each do |diff2|
  two_way_differs.product(three_way_differs).each do |diff2, diff3|
    describe "testing multiple types of conflicts" do
      subject { Dyph::Differ.merge(left, base, right,
                join_function: Dyph::Differ.standard_join,
                split_function: Dyph::Differ.split_on_new_line,
