...
describe Dyph::TwoWayDiffers::OutputConverter do
  let(:converter) { Dyph::TwoWayDiffers::OutputConverter }
  let(:differ)  { Dyph::TwoWayDiffers::HeckelDiff }

  describe "work with resig differ" do
    it "should be null" do
      expect(converter.convert_to_dyph_output([], [])).to eq []
    end
    ...
    it "should show an add" do
      ...
      converted_text = converter.convert_to_dyph_output(diff[:old_text], diff[:new_text])
