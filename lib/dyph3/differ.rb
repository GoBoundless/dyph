module Dyph3
  class Differ
    ...
    def self.merge(left, base, right, current_differ: default_differ, split_function: ->(x) { x } , join_function: ->(x) { x }, conflict_function: nil)
    ...
    def self.merge_text(left, base, right, current_differ: default_differ, split_function: split_on_new_line, join_function: standard_join, conflict_function: nil)
