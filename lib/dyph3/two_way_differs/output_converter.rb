module Dyph3
  module TwoWayDiffers
    module OutputConverter
      extend self
      def merge_results(calling_class, old_text, new_text)
        merged_text = []
        ...
        if (new_text.empty?)
          ...
          new_text.map.with_index.each do |line, i|
            if optional(line).text.value.nil?
              ...
              merged_text << {action: :no_change, line: line, old_index: line.row, new_index:  i}
              ((prev_no_change_old+1) ... old_text.length).each do |n|
                break if !optional(old_text[n]).text.value.nil?
                merged_text << {action: :delete, line: old_text[n], old_index: n+1, new_index: i+1}
              end
            end
          end
        end
      ...
      def convert_to_dyph3_output(merged_text)
        ...
        trimmed_result = pair_up_add_deletes collapsed_actions
        no_changes = trimmed_result.reject { |x| x[:action] == :no_change}
        t2 = set_offset(no_changes)
        t2.map { |r| Dyph3::Support::AssignAction.get_action(
          lo_a: r[:old_lo]-1 ,
          lo_b: r[:new_lo]-1,
          hi_a: r[:old_hi]-1,
          hi_b: r[:new_hi]-1
